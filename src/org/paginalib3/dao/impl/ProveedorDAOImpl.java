...
public class ProveedorDAOImpl implements ProveedorDAO {
    ...
    public List<Proveedor> listar() throws SQLException {
        ...
        try (Connection c = Conexion.getInstancia().conectar();
            ...
            while (r.next()) lista.add(new Proveedor(r.getString("nit_proveedor"), r.getString("nombre_proveedor"),
    ...
    @Override public boolean insertar(Proveedor p) throws SQLException { return guardar("sp_insertarproveedor", p); }
    @Override public boolean actualizar(Proveedor p) throws SQLException { return guardar("sp_actualizarproveedor", p); }

    private boolean guardar(String sp, Proveedor p) throws SQLException {
        validar(p);
        try (Connection c = Conexion.getInstancia().conectar();
             CallableStatement s = c.prepareCall("{CALL " + sp + "(?,?,?,?,?)}")) {
            s.setString(1, p.getNit().trim()); s.setString(2, p.getNombre().trim());
            s.setString(3, limpio(p.getTelefono())); s.setString(4, limpio(p.getDireccion()));
            s.setString(5, limpio(p.getCorreo())); s.execute(); return true;
        }
    }

    @Override
    public boolean desactivar(String nit) throws SQLException {
        if (nit == null || nit.isBlank()) throw new IllegalArgumentException("Selecciona un proveedor.");
        try (Connection c = Conexion.getInstancia().conectar();
             CallableStatement s = c.prepareCall("{CALL sp_eliminarproveedor(?)}")) {
            s.setString(1, nit.trim()); s.execute(); return true;
        }
    }

    private void validar(Proveedor p) {
        if (p == null || p.getNit() == null || p.getNit().isBlank()) throw new IllegalArgumentException("El NIT es obligatorio.");
    ...
    private String limpio(String s) { return s == null || s.isBlank() ? null : s.trim(); }
}
