...
public class LibroDAOImpl implements LibroDAO {
    ...
    public List<Libro> listarTodos() throws SQLException {
        ...
        try (Connection c = Conexion.getInstancia().conectar(); CallableStatement s = c.prepareCall("{CALL sp_listarlibros()}"); ResultSet r = s.executeQuery()) {
            while (r.next()) {
                lista.add(map(r));
            }
        }
    ...
    public List<Libro> buscar(String texto) throws SQLException {
        ...
        List<Libro> lista = new ArrayList<>();
        try (Connection c = Conexion.getInstancia().conectar(); CallableStatement s = c.prepareCall("{CALL sp_buscar_libros(?)}")) {
            s.setString(1, t);
    ...
    public Libro buscarPorIsbn(String isbn) throws SQLException {
        try (Connection c = Conexion.getInstancia().conectar(); CallableStatement s = c.prepareCall("{CALL sp_buscarlibro(?)}")) {
            s.setString(1, isbn);
            try (ResultSet r = s.executeQuery()) {
    ...
    public boolean insertar(Libro libro) throws SQLException {
        try (Connection c = Conexion.getInstancia().conectar(); CallableStatement s = c.prepareCall("{CALL sp_insertarlibro(?,?,?,?,?,?,?)}")) {
    ...
    public boolean insertarConStockInicial(Libro libro, int stockInicial, int idUsuario) throws SQLException {
        ...
        try (Connection c = Conexion.getInstancia().conectar(); CallableStatement s = c.prepareCall("{CALL sp_insertarlibro_stock_inicial(?,?,?,?,?,?,?,?,?)}")) {
    ...
    public boolean actualizar(Libro libro) throws SQLException {
        // El stock actual cambia únicamente mediante movimientos; el stock mínimo sí forma parte de la ficha del libro.
        try (Connection c = Conexion.getInstancia().conectar(); CallableStatement s = c.prepareCall("{CALL sp_actualizarlibro(?,?,?,?,?,?,?)}")) {
            cargarDatosLibro(s, libro);
            s.executeUpdate();
            return true;
        }
    ...
    public boolean cambiarEstado(String isbn, boolean activo) throws SQLException {
        try (Connection c = Conexion.getInstancia().conectar(); CallableStatement s = c.prepareCall("{CALL sp_cambiar_estado_libro(?,?)}")) {
            s.setString(1, isbn);
